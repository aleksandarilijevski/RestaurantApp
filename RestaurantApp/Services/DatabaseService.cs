...
namespace RestaurantApp.Services
{
    public class DatabaseService : IDatabaseService
    {
        ...
        {
            _efContext.Entry(onlineOrder).State = EntityState.Modified;
            await _efContext.SaveChangesAsync();
        }
        ...
        public async Task<List<Table>> GetAllTables()
        {
            ...
            List<Table> tables = await _efContext.Tables.ToListAsync();
            return tables;
        }
        ...
        public async Task<ArticleDetails> GetArticleDetailByArticleID(int id)
        {
            ...
            ArticleDetails articleDetails = await _efContext.ArticleDetails.Include(x => x.Article).FirstOrDefaultAsync(x => x.Article.ID == id);
        ...
        public async Task<List<ArticleDetails>> GetArticleDetailsByArticleID(int articleId)
        {
            ...
            List<ArticleDetails> articleDetails = await _efContext.ArticleDetails.Include(x => x.Article).Where(x => x.Article.ID == articleId).ToListAsync();
            return articleDetails;
        }
        ...
        public async Task<List<TableArticleQuantity>> GetTableArticleQuantitiesExceptProvidedID(Table table)
        {
            ...
            List<TableArticleQuantity> tableArticleQuantities = await _efContext.TableArticleQuantities.Select(x => x).Where(x => x.Table.ID != table.ID).ToListAsync();
